        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          {process.map((p, i) => (
            <motion.div
              ...
              whileHover={{ y: -6 }}
              className="group relative p-6 rounded-2xl bg-white border border-pink-100 shadow-sm shadow-pink-100 hover:shadow-md hover:shadow-pink-200 transition-all text-center flex flex-col items-center"
            >
              <div className="absolute inset-0 bg-gradient-to-br from-pink-500/5 via-transparent to-rose-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl" />

              <div className="relative z-10">
                <div className="text-3xl mb-3 font-black text-pink-600">{p.step}</div>
                <span className="text-[10px] font-bold text-pink-600 tracking-widest uppercase">{p.step}</span>
                <h3 className="text-base font-bold text-gray-900 mt-2 mb-2 group-hover:text-pink-600 transition-colors">{p.title}</h3>
